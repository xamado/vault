...
int win_add(int x, int y, int width, int height, int a4, int flags)
{
    ...
    w->buffer = (unsigned char*)mem_malloc(width * height * 4);
    if (w->buffer == NULL) {
        mem_free(w);
        return -1;
    }
    ...
    if (a4 == 256) {
        ...
        a4 = colorTable[GNW_wcolor[colorIndex]];
    }
    // a4 is now either 256 (texture) or a palette index (0-255)
    ...
    // Convert palette index to 32-bit RGBA for backgroundColor
    if (a4 != 256) {
    ...
    win_fill(index, 0, 0, width, height, w->backgroundColor);
    ...
    buf = w->buffer + (x + y * w->width) * 4;
        ...
        if (!(a6 & 0x02000000)) {
            if (w->backgroundColor == 256 && GNW_texture != NULL) {
                ...
                buf_fill(buf, v7, text_height(), w->width, w->backgroundColor);
            }
        }
        ...
        if ((a6 & 0xFF00) != 0) {
            ...
            unsigned char* pal = getColorPalette();
            int palIdx = colorTable[GNW_wcolor[colorIndex]];
            v27 = (0xFF << 24) | ((pal[palIdx * 3 + 2] << 2) << 16) | ((pal[palIdx * 3 + 1] << 2) << 8) | (pal[palIdx * 3] << 2);
        } else {
            v27 = a6;
        }

        FMtext_to_buf(buf, str, v7, w->width, v27);
...
void win_fill(int win, int x, int y, int width, int height, int a6)
{
    ...
    if (a6 == 256) {
        if (GNW_texture != NULL) {
            ...
            a6 = colorTable[GNW_wcolor[0]];
        }
    } else if ((a6 & 0xFF00) != 0) {
        int colorIndex = (a6 & 0xFF) - 1;
        a6 = colorTable[GNW_wcolor[colorIndex]];
    }
    ...
    unsigned int color32 = (alpha << 24)
        ...
        | ((pal[palIdx * 3 + 1] << 2) << 8)
        | (pal[palIdx * 3] << 2);
    buf_fill(w->buffer + (w->width * y + x) * 4, width, height, w->width, color32);
}
...
// 0x4D6FD8
void GNW_win_refresh(Window* w, Rect* rect, unsigned char* destBuf)
{
...

    {
        ...
        if (v26->rect.lrx >= v26->rect.ulx && v26->rect.lry >= v26->rect.uly) {
                    ...
                    if (dest_pitch != 0) {
                    ...
                    } else {
                        {
                            ...
                            if (buf32 != NULL) {
                                ...
                                GNW95_ShowRect((unsigned char*)buf32, width, height, 0, 0, width, height, v16->rect.ulx, v16->rect.uly);
                                mem_free(buf32);
                            }
                        }
                    }
            ...
            if (!doing_refresh_all && destBuf == NULL && mouse_hidden() == 0) {
